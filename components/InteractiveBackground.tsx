...
export default function InteractiveBackground({ className }: { className?: string }) {
  ...
  const initFluid = useCallback((canvas: HTMLCanvasElement) => {
    ...
    const gl = canvas.getContext("webgl2", { alpha: true, depth: false, stencil: false, antialias: false, preserveDrawingBuffer: false }) as WebGL2RenderingContext;
    if (!gl) return;
    ...
    function compileShader(gl: WebGL2RenderingContext, type: number, source: string, keywords?: string[]) {
    ...
    const displayMaterial = new GLProgram(baseVertexShader, compileShader(gl, gl.FRAGMENT_SHADER, displayShaderSource));
    ...
    function updatePointerDownData(pointer: ReturnType<typeof pointerPrototype>, id: number, posX: number, posY: number) {
      pointer.id = id;
      pointer.down = true;
      pointer.moved = false;
    ...
    function updatePointerMoveData(pointer: ReturnType<typeof pointerPrototype>, posX: number, posY: number, color?: { r: number; g: number; b: number }) {
      pointer.prevTexcoordX = pointer.texcoordX;
      pointer.prevTexcoordY = pointer.texcoordY;
      pointer.texcoordX = posX / canvas.width;
      pointer.texcoordY = 1.0 - posY / canvas.height;
    ...
    canvas.addEventListener("mousedown", e => {
      ...
      if (!pointer) { pointer = pointerPrototype(); pointers.push(pointer); }
    ...
    canvas.addEventListener("mousemove", e => {
      ...
      pointer.down = true; // Allow movement without click
    });
    ...
    function drawDisplay(target: WebGLFramebuffer | null) {
      const width = target == null ? gl.drawingBufferWidth : (target as any).width;
      const height = target == null ? gl.drawingBufferHeight : (target as any).height;
    ...
    function update() {
      ...
      if (resizeCanvas()) { /* skip frame on resize */ }
  ...
  return (
    <div className={className || "absolute inset-0 z-0 bg-black pointer-events-none"}>
      <canvas
        ref={canvasRef}
